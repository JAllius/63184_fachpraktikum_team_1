...
export default function ExplainabilityRegression({ summary }: Props) {
  const [grouped, setGrouped] = useState(true);

  const meanAbsParent = summary.global.mean_abs_parent;
  const meanAbsFeature = summary.global.mean_abs;
  const perFeature = summary.distributions.per_feature;
  ...
  const barData = useMemo(() => {
    ...
    // already top-k; but labels can be longer -> chart needs more height/left space
  ...
  // height depends on number of bars (prevents overlap/“displaced” bars)
  const barHeight = Math.min(520, Math.max(220, barData.length * 22)); // smaller overall

  // SCATTER DATA: x=typical value, y=impact
  ...
  const xDomain = useMemo(() => {
    ...
    const pad = (max - min) * 0.08 || 1; // handle flat values
        ...
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Bigger bars mean this factor matters more.
            </p>
          </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Typical value vs. impact on the outcome.
            </p>
          </div>
